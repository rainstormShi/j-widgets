...
(function (document){
  ...
  _ = {
    ...
    ,isIdentityCard:function(cardId){
      ...
      var iSum=0 ;  
      var info="" ;  
      if(!/^\d{17}(\d|x)$/i.test(cardId)) return "请输入正确的身份证号码2";
      cardId=cardId.replace(/x$/i,"a");  
      if(aCity[parseInt(cardId.substr(0,2))]==null) return "请输入正确的身份证号码1";
      sBirthday=cardId.substr(6,4)+"-"+Number(cardId.substr(10,2))+"-"+Number(cardId.substr(12,2));  
      var d=new Date(sBirthday.replace(/-/g,"/")) ;  
      if(sBirthday!=(d.getFullYear()+"-"+ (d.getMonth()+1) + "-" + d.getDate()))return "请输入正确的身份证号码";
      // 以下验证为准确验证 一般情况 不考虑(如测试时)
      // for(var i = 17;i>=0;i --) iSum += (Math.pow(2,i) % 11) * parseInt(cardId.charAt(17 - i),11) ;  
      // if(iSum%11!=1) return alert("你输入的身份证号非法");  
